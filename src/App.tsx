...
export default function App() {
  ...
  const [loggedMeals, setLoggedMeals] = useState<LoggedMealItem[]>(() => {
    // Initial meals: prior morning meals + the 250g Dosa lunch (actual intake)
    return [
      ...INITIAL_LOGGED_MEALS,
      {
  ...
  const handleUpdateMealMath = (updated: Partial<MealIntakeMath>) => {
    setMealMath((prev) => {
      const merged = { ...prev, ...updated };
      // Also update the lunch logged meal item in state
  ...
  const handleAddRecommendation = (rec: RegionalRecommendation) => {
  ...
  const handleAddVoiceItems = (items: LoggedMealItem[]) => {
    setLoggedMeals((prev) => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col items-center selection:bg-lime-200 selection:text-lime-900 font-sans">
      {/* Offline Status Badge */}
      <OfflineIndicator />

      {/* Centered Mobile Frame (max-w-md on desktop) */}
      <div className="w-full max-w-md min-h-screen bg-lime-50/40 border-x border-lime-200/80 shadow-2xl flex flex-col relative pb-32">
        ...
        {/* Free Navigation Tabs (Always accessible when tour banner minimized or user wants quick jumps) */}
        <div className="bg-white/90 border-b border-lime-200/80 px-3 py-2 flex items-center gap-1.5 overflow-x-auto scrollbar-none">
          {WALKTHROUGH_STEPS.map((s) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-full whitespace-nowrap font-medium transition ${
                  isActive
