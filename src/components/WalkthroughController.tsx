...
export const WalkthroughController: React.FC<WalkthroughControllerProps> = ({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 px-3 pb-3 pt-2 bg-gradient-to-t from-slate-900/40 via-slate-900/20 to-transparent pointer-events-none">
      <div className="max-w-md mx-auto pointer-events-auto flex flex-col gap-2">
        {/* Explanatory Callout Banner */}
        {isTourMode && (
          <div className="bg-slate-900/95 text-white backdrop-blur-md rounded-2xl p-3 shadow-xl border border-lime-400/40 transition-all duration-300">
        ...
        <div className="bg-white/95 backdrop-blur-md rounded-2xl p-2 shadow-2xl border border-lime-300/80 flex items-center justify-between gap-2">
          {/* Previous Button */}
          <button
            onClick={onPrev}
          ...
          <div className="flex flex-col items-center justify-center px-1">
            <div className="flex items-center gap-1">
              ...
              <span className="text-[11px] text-lime-700 font-semibold truncate max-w-[110px]">
                {stepInfo.shortTitle}
              </span>
            </div>
            {/* Step Dots */}
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              title={isTourMode ? 'Free Explore Mode' : 'Resume Guided Tour'}
            >
            ...
            {currentStep < totalSteps ? (
              <button
                onClick={onNext}
                className="flex items-center gap-1 px-3.5 py-2 rounded-xl text-xs font-bold bg-lime-500 hover:bg-lime-600 active:scale-95 text-slate-900 shadow-sm transition"
              >
                <span>Next</span>
